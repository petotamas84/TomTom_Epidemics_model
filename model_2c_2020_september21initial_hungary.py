...
inf = 20000.0         #starting value of infected people
...
while(inf < pop and ill >= 2):
    ...
    if(ri_e > 1):
        ...
        n_inf = (inf - rec_c - n_death)*(ri_e - 1)  # number of new infection
        n_inf = (n_inf // 1) - imm_c
        ...
        n_inf = (inf - rec_c - n_death)*(1 - ri_e)
        n_inf = (n_inf // 1) - imm_c
...
filename_1 = '2_model_2020_autumn_hungary_figure_1' + str(ri) + '_' + str(rate_i) + '_' + str(rate_d) + '.png'
filename_2 = '2_model_2020_autumn_hungary_figure_2' + str(ri) + '_' + str(rate_i) + '_' + str(rate_d) + '.png'
...
plt.plot(dd_t, n_inf_t, 'yo', markersize = 5)
plt.plot(dd_t, n_death_t, 'ro', markersize = 5)
plt.plot(dd_t, n_death_t, 'bo', markersize = 5)
plt.plot(dd_t, n_death_t, 'go', markersize = 5)
plt.ylabel("Daily infected - yellow\n Daily ill - red\n Daily deseases - black\n Daily recovered - green", fontsize = 8)
plt.xlabel("Days", fontsize = 12)
plt.title("Number of infected, patients, deceased and recovered (Daily)\n(R = %.2f, Disease rate = %.2f, Mortality = %.3f)" %(ri, rate_i, rate_d), fontsize=14)
plt.show()
