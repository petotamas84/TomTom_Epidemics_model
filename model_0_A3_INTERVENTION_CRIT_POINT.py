...
if(ACT_TYPE == 1):
    ...
    ACT_POINT_I = input("Enter the number of daily hospitalisation case when you interrupt: ")
...
while(inf < pop and ill >= 2):
    ...
    hosp = ill * rate_h    #new disease
    
    if(hosp < 0):
        
        hosp = 0
    
    if(hosp % 2 < (hosp / 2)): #egészre kerekítés
            
        hosp = hosp // 1

    else:
            
        hosp = (hosp // 1) + 1 #egészre kerekítés vége
    ...
    if (hosp/rc >= ACT_POINT_I):
        
        CLI = 0
        ACT_START = ACT_START + rc
    
    sum_hosp = sum_hosp + hosp
    
    if (hosp > CRIT_POINT):
    
        n_death = (ill - (hosp - CRIT_POINT)) * rate_d + (hosp - CRIT_POINT) * rate_d * CRIT_RATE     #new deceased
...
plt.subplot(512)
plt.plot(dai_t, ill_t, 'mo', markersize = 1)
plt.ylabel("Number of daily new patients", fontsize = 5)

plt.subplot(513)
plt.plot(daho_t, hosp_t, 'ro', markersize = 1)
